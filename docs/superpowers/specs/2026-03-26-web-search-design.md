...
La ricerca web viene esposta come **tool nativo Agno** (`DuckDuckGoTools`) aggiunto al solo `build_chat_agent()` in `agent.py`. L'agente usa il tool solo su richiesta esplicita dell'utente grazie a un'istruzione dedicata. La UI in `chat_app.py` cattura l'evento di completamento del tool e mostra gli URL estratti in una sezione separata "🌐 Fonti web".

```
rag_preventivi/
  agent.py        ← aggiunge DuckDuckGoTools + istruzione a build_chat_agent()
  chat_app.py     ← cattura RunEvent.tool_call_completed, mostra 🌐 Fonti web
  requirements.txt ← aggiunge duckduckgo-search
```
...
|------|--------|----------------|
| `rag_preventivi/agent.py` | Modifica | Aggiunge `tools=[DuckDuckGoTools()]` e istruzione a `build_chat_agent()` |
| `rag_preventivi/chat_app.py` | Modifica | Cattura `RunEvent.tool_call_completed`, estrae URL, mostra `🌐 Fonti web` |
| `rag_preventivi/requirements.txt` | Modifica | Aggiunge `duckduckgo-search` |

## Modifiche a agent.py

`build_chat_agent()` riceve:
- `tools=[DuckDuckGoTools()]` — import: `from agno.tools.duckduckgo import DuckDuckGoTools`
...
Nel loop di streaming, si aggiunge la gestione di `RunEvent.tool_call_completed`:
...
web_sources = []  # inizializzato insieme a sources e full_text
    ...
    if tool and "duckduckgo" in (getattr(tool, "tool_name", "") or "").lower():
...
`duckduckgo-search` — pacchetto Python puro, nessuna API key richiesta.

## Gestione errori

- Se DuckDuckGo non restituisce risultati: l'agente lo comunica nel testo della risposta, nessun expander "🌐 Fonti web" appare.
- Se il parsing degli URL fallisce: `web_sources` rimane vuota, nessun crash.
- Se DuckDuckGo è temporaneamente non raggiungibile: l'eccezione viene catturata dal `try/except` esistente in `chat_app.py` e mostrata come `_Errore durante la risposta: ..._`.
